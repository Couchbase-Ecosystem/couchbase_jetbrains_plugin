...
public class PillowFightDialog extends DialogWrapper {
    private static final String NETWORK_CONNECTIVITY_ERROR_MESSAGE = "Could not connect to the cluster. Please check "
            + "your network connectivity, if the cluster is active or if the credentials are still valid.";
    ...
    protected PillowFightDialog(Project project, String bucket, String durability, String persistTo, String batchSize
            , String numberItems, String keyPrefix, String numberThreads, String percentage, String noPopulation,
                                String populateOnly, String minSize, String maxSize, String numberCycles,
                                String sequential, String startAt, String timings, String expiry, String replicateTo,
                                String lock, String json, String noop, String subdoc, String pathcount) {
        this(project);
    ...
    private static void showBalloonHint(JBLabel questionMark, String hintText) {
        ...
        balloonHint =
                factory.createBalloonBuilder(textArea).setFillColor(UIManager.getColor("ToolTip.background")).setAnimationCycle(0).createBalloon();
    ...
    private void handleBucketComboBoxChange() {
        try {
            String selectedBucket =
                    Optional.ofNullable(bucketComboBox.getSelectedItem()).map(Object::toString).orElse(null);

            Set<String> scopeNamesSet =
                    ActiveCluster.getInstance().get().bucket(selectedBucket).collections().getAllScopes().stream().map(ScopeSpec::name).collect(Collectors.toSet());
    ...
    private void handleScopeComboBoxChange() {
        try {
            ...
            String selectedScope =
                    Optional.ofNullable(scopeComboBox.getSelectedItem()).map(Object::toString).orElse(null);
            ...
            Consumer<String> refreshCollectionCombo = scope -> {
                String[] collectionNamesArray =
                        ActiveCluster.getInstance().get().bucket(selectedBucket).collections().getAllScopes().stream().filter(s -> s.name().equals(scope)).flatMap(s -> s.collections().stream()).map(CollectionSpec::name).distinct().toArray(String[]::new);
            ...
            if (collectionComboBox.getItemCount() > 0) {
                ...
                Messages.showErrorDialog("The selectd scope is empty. Selecting default scope and collection", "Empty"
                        + " Bucket");
                scopeComboBox.setSelectedItem(DEFAULT_TAG);
                collectionComboBox.setSelectedItem(DEFAULT_TAG);
            }
    ...
    protected @Nullable JComponent createCenterPanel() {
        ...
        panel.add(createLabelWithBalloon("Bucket:        ",
                "A bucket is the fundamental space for storing data in " + "Couchbase Server"), gbc);
        ...
        gbc.gridx--;
        gbc.gridy++;
        panel.add(createLabelWithBalloon("Scope:        ",
                "A scope is a logical grouping of collections within a " + "bucket"), gbc);
        ...
        gbc.gridx--;
        gbc.gridy++;
        panel.add(createLabelWithBalloon("Collection:        ", "A collection is a data container within a scope"),
                gbc);
        ...
        gbc.gridx--;
        gbc.gridy++;
        panel.add(createLabelWithBalloon("Durability:        ", "Specify durability level for mutation operations"),
                gbc);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Persist-to: ",
                "Wait until the item has been persisted to at " + "least NUMNODES nodes' disk. If NUMNODES is 1 then "
                        + "wait until only the master node has persisted the " + "item for this key. You may not " +
                        "specify a number greater than the number of nodes actually in the " + "cluster. -1 is " +
                        "special value, which mean to use all available nodes."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Batch Size: ",
                "This controls how many commands are scheduled " + "per cycles. To simulate one operation at a time, "
                        + "set this value to 1."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Number of Items: ", "Set the total number of items the workload "
                + "will access within the cluster. This will also determine the working set size at the server and " + "may " + "affect disk latencies if set to a high number."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Key Prefix: ",
                "Set the prefix to prepend to all keys in the " + "cluster. Useful if you do not wish the items to " + "conflict with existing data."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Number of Threads: ",
                "Set the number of threads (and thus the " + "number of client instances) to run concurrently. Each " + "thread is assigned its own client object."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Percentage", "The percentage of operations which should be " +
                "mutations. A value of 100 means only mutations while a value of 0 means only retrievals."),
                gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("No Population: ", "By default cbc-pillowfight will load all the "
                + "items (see --num-items) into the cluster and then begin performing the normal workload. " +
                "Specifying" + " " + "this option bypasses this stage. Useful if the items have already been loaded " +
                "in a previous " + "run."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Populate Only: ",
                "Stop after population. Useful to populate " + "buckets with large amounts of data."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Min Size: ",
                "Specify the minimum size to be stored into the " + "cluster. This is typically a range, in which " + "case each value generated will be between Min Size and " + "Max Size bytes."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Max Size: ",
                "Specify the maximum size to be stored into the " + "cluster. This is typically a range, in which " + "case each value generated will be between Min Size and " + "Max Size bytes."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Number of Cycles: ",
                "Specify the number of times the workload " + "should cycle. During each cycle an amount of " +
                        "--batch-size operations are executed. Setting this to " + "-1 will cause the workload to " +
                        "run" + " infinitely."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Sequential: ",
                "Specify that the access pattern should be done " + "in a sequential manner. This is useful for " +
                        "bulk-loading many documents in a single server."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Start At: ", "This specifies the starting offset for the items. "
                + "The items by default are generated with the key prefix (--key-prefix) up to the number of items " + "(--num-items). The --start-at value will increase the lower limit of the items. This is useful to " + "resume a previously cancelled load operation."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Timings: ",
                "Enabled timing recorded. Timing histogram will be " + "dumped to STDERR on SIGQUIT (CTRL-/). When " + "specified second time, it will dump a histogram of " + "command timings and latencies to the" + " screen every second."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Expiry: ", "Set the expiration time on the document for SECONDS "
                + "when performing each operation. Note that setting this too low may cause not-found errors to " +
                "appear " + "on the screen."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Replicate To: ", "Wait until the item has been replicated to at "
                + "least NREPLICAS replica nodes. The bucket must be configured with at least one replica, and at " + "least" + " NREPLICAS replica nodes must be online. -1 is special value, which mean to use all " + "available " + "replicas."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Lock: ",
                "This will retrieve and lock an item before update, " + "making it inaccessible for modification " +
                        "until the update completed, or TIME has passed."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("JSON: ",
                "Make pillowfight store document as JSON rather than " + "binary. This will allow the documents to " + "nominally be analyzed by other Couchbase services such as " + "Query and MapReduce."), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("NOOP: ", "Use couchbase NOOP operations when running the " +
                "workload. This mode ignores population, and all other document operations. Useful as the most " +
                "lightweight workload."), gbcCollapsable);
        ...
        collapsiblePanel.add(createLabelWithBalloon("Subdoc: ",
                "Use couchbase sub-document operations when running " + "the workload. In this mode pillowfight will "
                        + "use Couchbase sub-document operations to perform gets " + "and sets of data. This option " + "must be used with --json"), gbcCollapsable);
        ...
        gbcCollapsable.gridx--;
        gbcCollapsable.gridy++;
        collapsiblePanel.add(createLabelWithBalloon("Pathcount: ", "Specify the number of paths a single " + "sub" +
                "-document" + " operation should contain. By default, each subdoc operation operates on only a " +
                "single path within " + "the document. You can specify multiple paths to atomically executed " +
                "multiple" + " subdoc operations " + "within a single command."), gbcCollapsable);
        ...
        panel.setBorder(new EmptyBorder(0, 15, 0, 20));
        ...
        collapsiblePanel1.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                if (collapsiblePanel1.contentIsEnabled()) {
                    jbScrollPane.setPreferredSize(new Dimension(jbScrollWidth,
    ...
    protected void doOKAction() {
        ...
        if (errors.isEmpty()) {
            ...
            try {
                ...
                pillowFightRunner.runPillowFightCommand(project, getValue(bucketComboBox), getValue(scopeComboBox),
                        getValue(collectionComboBox), getValue(durabilityComboBox), getValue(persistToSpinner),
                        getValue(batchSizeSpinner), getValue(numberItemsSpinner), keyPrefixTextField.getText(),
                        getValue(numberThreadsSpinner), getValue(percentageSpinner), getValue(noPopulationComboBox),
                        getValue(populateOnlyComboBox), getValue(minSizeSpinner), getValue(maxSizeSpinner),
                        getValue(numberCyclesSpinner), getValue(sequentialComboBox), getValue(startAtSpinner),
                        getValue(timingsComboBox), getValue(expirySpinner), getValue(replicateToSpinner),
                        getValue(lockSpinner), getValue(jsonComboBox), getValue(noopComboBox),
                        getValue(subdocComboBox), getValue(pathcountSpinner));
