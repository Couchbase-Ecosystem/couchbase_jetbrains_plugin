...
public class PillowFightDialog extends DialogWrapper {
    ...
    private static void showBalloonHint(JBLabel questionMark, String hintText) {
        ...
        balloonHint = factory.createBalloonBuilder(textArea).setFillColor(UIManager.getColor("ToolTip.background"))
                .setAnimationCycle(0).createBalloon();
    ...
    protected @Nullable JComponent createCenterPanel() {
        ...
        collapsiblePanel.add(
                createLabelWithBalloon("Populate Only: ",
        ...
        if (errors.isEmpty()) {
            ...
            try {
                ...
                pillowFightRunner.runPillowFightCommand(
                        project,
                        getValue(bucketComboBox),
                        getValue(scopeComboBox),
                        getValue(collectionComboBox),
                        getValue(durabilityComboBox),
                        getValue(persistToSpinner),
                        getValue(batchSizeSpinner),
                        getValue(numberItemsSpinner),
                        keyPrefixTextField.getText(),
                        getValue(numberThreadsSpinner),
                        getValue(percentageSpinner),
                        getValue(noPopulationComboBox),
                        getValue(populateOnlyComboBox),
                        getValue(minSizeSpinner),
                        getValue(maxSizeSpinner),
                        getValue(numberCyclesSpinner),
                        getValue(sequentialComboBox),
                        getValue(startAtSpinner),
                        getValue(timingsComboBox),
                        getValue(expirySpinner),
                        getValue(replicateToSpinner),
                        getValue(lockSpinner),
                        getValue(jsonComboBox),
                        getValue(noopComboBox),
                        getValue(subdocComboBox),
                        getValue(pathcountSpinner));
